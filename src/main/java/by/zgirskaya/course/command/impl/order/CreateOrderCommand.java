  ...
  public void execute(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (currentOrder == null) {
        session.setAttribute("error", "No items in cart to create order");
        response.sendRedirect(request.getContextPath() + "/cart");
        return;
      }

      var items = orderService.getOrderItems(currentOrder.getId());
      if (items == null || items.isEmpty()) {
        session.setAttribute("error", "Cannot create empty order");
        response.sendRedirect(request.getContextPath() + "/cart");
        return;
      }
      ...
      if (success) {
        session.setAttribute("successMessage",
            String.format("Order created successfully! Delivery date: %s",
      ...
      session.setAttribute("error", "Invalid parameter format: " + e.getMessage());
      response.sendRedirect(request.getContextPath() + "/orders/new");
    }
  }
}